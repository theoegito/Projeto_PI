...
// Explosão (usa sheet horizontal; cada quadro é quadrado = altura da textura)
static const float EXP_FPS   = 4.0f;   // velocidade da explosão
static const float EXP_SCALE = 120.0f;  // tamanho final (lado) na tela (≈ tamanho do boss)
...
void Phase2_Update(Phase2 *ph, float dt, Assets *a){
    // Se estiver tocando a explosão, só atualiza a animação e sai
    if (ph->exploding){
        ph->expTime += dt;
        float frameTime = 1.0f / EXP_FPS;

        // quantos frames tem? largura/altura (sheet horizontal com frames quadrados)
        int frameSize = a->texExplosion.height; // 32
        int totalFrames = a->texExplosion.width / frameSize; // ex: 192/32 = 6
        ...
        return; // congela o resto enquanto explode
    }
...
void Phase2_Draw(Phase2 *ph, Assets *a){
   ...
   // Boss (só quando NÃO está na explosão)
...
for (int i = 0; i < MAX_BOSS_BULLETS; i++) if (ph->bbul[i].active)
    ...
    if (ph->exploding){
        ...
        // desenha centrado e ampliado para ~120x120 (tamanho do boss)
        float s = EXP_SCALE; // 120
